...
**Tuist 기반 멀티 모듈 구조**에 **TCA 네이티브 구조 + `swift-dependencies` DI 라이브러리 기반 클라이언트 패턴**을 적용했습니다.

- **App** — 앱 타겟 + ShareExtension
- **BKFeatures** — TCA 기반 Feature 레이어
- **BKCore** — `NetworkCore` + `@DependencyClient` 기반 Clients
- **BKDesignSystem** — 디자인 시스템 및 리소스
- **BKShared** — 공용 확장·모델(`BKCommon`, `BKModel`)
