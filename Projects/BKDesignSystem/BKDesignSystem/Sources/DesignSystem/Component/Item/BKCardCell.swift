...
public struct BKCardCell: View {
  ...
  private func makeHeaderIconsSection() -> some View {
    HStack(alignment: .center, spacing: 16) {
      Button {
        ...
        BKIcon(image: isMarked ? BKDesignSystem.Images.icoSaveClcik : BKDesignSystem.Images.icoSave, color: .bkColor(.gray900), size: CGSize(width: 20, height: 20))
      }
      
      Button {
        menuAction()
      } label: {
        BKIcon(image: BKDesignSystem.Images.icoMoreVertical, color: .bkColor(.gray600), size: CGSize(width: 20, height: 20))
      }
    }
  }
  ...
  private func makeAddFolerButton() -> some View {
    BKIcon(image: BKDesignSystem.Images.icoPlus, color: .bkColor(.main300), size: CGSize(width: 14, height: 14))
      .padding(.vertical, 7)
      .padding(.horizontal, 12)
      .background(
        Circle()
