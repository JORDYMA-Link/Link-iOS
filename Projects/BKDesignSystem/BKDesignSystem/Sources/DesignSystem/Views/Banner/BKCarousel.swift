...
@available(iOS 18.0, *)
struct BKCarousel<Content>: View where Content: View {
  ...
  @State private var timer = Timer.publish(every: 3.0, on: .main, in: .default).autoconnect()
    ...
    GeometryReader {
      ...
      Group(subviews: content) { collection in
        ...
        .onScrollPhaseChange{ oldPhase, newPhase in
          ...
          if scrollPosition == collection.count && !isScrolling && !isHoldingScreen {
            scrollPosition = 0
          }
        }
        ...
        .onChange(of: isHoldingScreen, { oldValue, newValue in
          if newValue {
            ...
            timer = Timer.publish(every: 3.0, on: .main, in: .default).autoconnect()
          }
        })
