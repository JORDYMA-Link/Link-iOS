              ...
              LazyVStack(spacing: 20) {
                Section {
                  ForEach(LinkCard.mock(), id: \.id) { item in
                    BKCardCell(sourceTitle: item.sourceTitle, sourceImage: "", isMarked: true, saveAction: {}, menuAction: {}, title: item.title, description: item.description, keyword: item.keyword)
